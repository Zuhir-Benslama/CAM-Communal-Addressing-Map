# Security — CAM (Communal Addressing Map) Plugin
...
| **JWT auth non-functional** — `secrets.token_hex(16)` generated ephemeral key never stored, tokens unverifiable | Fixed secret from `CAM_JWT_SECRET` env var with fallback | `core.py` |
...
| **Unvalidated DB restore** — arbitrary `.sqlite` files copied over production DB | SQLite magic bytes header validation | `CAM_dialog.py` |
...
| **Env var injection** — `PYTHON_QGIS_BAT`, `SOFFICE_EXE` passed to subprocess without validation | `os.path.isfile()` + `os.access(X_OK)` checks | `CAM_dialog.py`, `reporting.py` |
| **SQL logging enabled** — `echo=True` leaked query data to stdout | Changed to `echo=False` | `models.py` |
| **Missing input validation** — user text fields accepted arbitrary input | `_validate_text()` strips + caps length on all `QLineEdit.text()` reads | `CAM_dialog.py`, `PopupDialog.py` |
...
Report security issues to zuhirbenslama@protonmail.com.
